// HNSWSQ/MPMI + ROrder batch-search perf test - efSearch=512
...
static std::vector<PerfEventGroup> get_perf_event_groups() {
    return {
...
static pid_t start_perf_stat(pid_t target_pid, const std::string& events, const std::string& output_file) {
    pid_t perf_pid = fork();
    if (perf_pid == 0) {
        freopen(output_file.c_str(), "w", stderr);
        execlp("perf", "perf", "stat", "-e", events.c_str(),
               "-p", std::to_string(target_pid).c_str(), nullptr);
        _exit(127);
    }
    return perf_pid;
}

static void stop_perf_stat(pid_t perf_pid) {
    if (perf_pid > 0) {
...
static std::string sanitize_tag(const std::string& s) {
    std::string t = s; for (auto& c : t) if (c=='/' || c=='\\' || c==' ' || c==':') c = '_'; return t;
}
...
static std::vector<std::vector<int>> get_groundtruth(
    ...
    const std::string& gt_file_config,
    faiss::MetricType metric,
    long& gt_ms_out)
{
    auto t0 = std::chrono::high_resolution_clock::now();
...
// -------------------- metrics --------------------
static faiss::idx_t map_id_if_needed(
    faiss::idx_t id,
    const faiss::IndexHNSWSQ* hnsw,
    bool use_map)
{
    if (!use_map || !hnsw || id < 0) return id;
    try {
...
struct RunOutcome {
    ...
    long build_ms = 0, gt_ms = 0, search_ms = 0, load_ms = 0, reorder_ms = 0;
};
...
// -------------------- 批查询测试（带perf）--------------------
static RunOutcome measure_batch_with_perf(
    faiss::Index* index,
    int efSearch,
    const std::vector<float>& query_vectors,
    int dim, int nq, int k,
    ...
    bool use_reorder_map)
{
    ...
    const auto* hnsw_sq = dynamic_cast<const faiss::IndexHNSWSQ*>(index);
    ...
    for (size_t i = 0; i < event_groups.size(); i++) {
        ...
        std::cout << "[Perf " << (i + 1) << "/" << event_groups.size() << "] "
                  << event_groups[i].name << " ..." << std::flush;
    ...
    const RunOutcome& out)
{
    ...
    f << "dataset,nb,nq,dim,M,efC,efS,k,warmup_rounds,warmup_q,"
      << "reorder_algo,qps,recall,build_ms,reorder_ms,load_ms,gt_ms,search_ms\n";
    }
    f << dataset_name << "," << nb << "," << nq << "," << dim << ","
      ...
      << out.build_ms << "," << out.reorder_ms << "," << out.load_ms << ","
      << out.gt_ms << "," << out.search_ms << "\n";
}
...
static Cli parse_cli(int argc, char** argv) {
    ...
    for (int i=1;i<argc;i++) {
        ...
        else if (a=="--index") c.index_file = next();
        else if (a=="--reorder") {
            auto algo = next();
            if (algo == "none" || algo == "0" || algo == "off") c.reorder_algo.clear();
            else c.reorder_algo = algo;
        }
        else if (a=="--reorder_map") c.use_reorder_map = (std::stoi(next()) != 0);
        ...
        else if (a=="--metric") c.metric = next();
...
static std::vector<DatasetDesc> all_presets() {
    return {
        {"SIFT1M",  "vectors_out/sift1m_base.bin",  "vectors_out/sift1m_query.bin",
         "vectors_out/sift1m_groundtruth.ivecs",
         "vectors_out/index_cache/SIFT1M_hnswsq_nb1000000_dim128_M16_efC256_l2_sift1m_base.bin_512000000.faiss",
         "l2", 128},
        {"GIST1M",  "vectors_out/gist1m_base.bin",  "vectors_out/gist1m_query.bin",
         "vectors_out/gist1m_groundtruth.ivecs",
         "vectors_out/index_cache/GIST1M_hnswsq_nb1000000_dim960_M16_efC256_l2_gist1m_base.bin_3840000000.faiss",
         "l2", 960}
    };
}
                            ...
                            const Cli& cli)
{
    ...
    if (!cli.reorder_algo.empty()) {
        ...
        const auto& perm = hnsw_sq->get_reorder_perm();
        if (perm.size() != (size_t)nb) {
            std::cerr << "  警告: reorder_perm 大小=" << perm.size()
                      << " 与 nb=" << nb << " 不一致，Recall ID 映射可能异常\n";
        }
    } else {
        std::cout << "ROrder: disabled (--reorder none)\n";
    }
    ...
    auto out = measure_batch_with_perf(index.get(), efSearch, query, dim, nq, cli.k, gts,
                                       !cli.reorder_algo.empty() && cli.use_reorder_map);

    out.build_ms = 0;  // 使用预构建索引,无构建时间
    out.load_ms = load_ms;
    out.reorder_ms = reorder_ms;
    out.gt_ms = gt_ms;

    // 输出结果
    std::cout << "\n========== 测试结果 ==========\n";
    std::cout << "Reorder: " << (cli.reorder_algo.empty() ? "none" : cli.reorder_algo)
              << " (" << reorder_ms << " ms)\n";
    ...
    // 写CSV
    append_result_csv(name, nb, nq, dim, cli.M, cli.efC, efSearch, cli.k,
                      cli.warmup_rounds, std::min(cli.warmup_q, nq), cli.reorder_algo, out);
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (cli.dataset == "CUSTOM") {
            ...
            run_one_dataset(cli.name, cli.base_file, cli.query_file, cli.gt_file,
                          cli.index_file, cli.metric, cli.dim, cli);
        ...
        if (cli.dataset == "ALL") {
            ...
            for (const auto& ds : presets) {
                run_one_dataset(ds.name, ds.base_file, ds.query_file, ds.gt_file,
                              ds.index_file, ds.metric, ds.dim, cli);
            }
            std::cout << "\n结果已追加到: " << result_dir() << "/hnsw_batch512_perf.csv\n";
            return 0;
        }
