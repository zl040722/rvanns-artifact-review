// test_hnsw_batch512_perf.cpp
// HNSW 批查询 Perf测试 - efSearch=512
// 特性：
// 1. 只测试 efSearch=512
// 2. 批查询模式：search(nq) 一次性查询所有query
// 3. 支持 RISC-V perf 事件监控
// 4. 输出批查询 QPS 和 Recall
...
static std::vector<DatasetDesc> all_presets() {
    return {
        {"SIFT1M",  "vectors_out/sift1m_base.bin",  "vectors_out/sift1m_query.bin",  
         ...
         "vectors_out/index_cache/SIFT1M_hnsw_nb1000000_dim128_M16_efC256_l2_sift1m_base.bin_512000000.faiss",
         "l2", 128},
        ...
        {"Cohere10M-IP", "vectors_out/unit_cohere_10m_base.bin", "vectors_out/unit_cohere_10m_query.bin",
         ...
         "vectors_out/index_cache/Cohere10M-IP-GTinternal-20251109_hnsw_nb10000000_dim768_M16_efC256_ip_unit_cohere_10m_base.bin_30720000000.faiss",
         "ip", 768}
    };
}
                            ...
                            const Cli& cli)
{
    ...
    append_result_csv(name, nb, nq, dim, cli.M, cli.efC, efSearch, cli.k,
                      cli.warmup_rounds, std::min(cli.warmup_q, nq), out);
}
        ...
        if (cli.dataset.empty()) {
            ...
            if (it == presets.end()) {
                std::ostringstream oss; oss << "未知 --dataset: " << cli.dataset
                    << "（可选：SIFT1M|GIST1M|Cohere10M-IP|ALL|CUSTOM）";
                throw std::runtime_error(oss.str());
            }
            const auto& ds = *it;
            run_one_dataset(ds.name, ds.base_file, ds.query_file, ds.gt_file, 
