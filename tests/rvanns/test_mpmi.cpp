...
static std::string
generate_index_cache_filename_hnswsq(const std::string& name, int dim, int nb, int M, int efC,
...
// -------------------- metrics & mapping --------------------
static inline faiss::idx_t
map_id_if_needed(faiss::idx_t id, const faiss::IndexHNSWSQ* hnsw, bool use_map) {
...
static double
recall_at_k_one_maybe_map(const faiss::idx_t* pred_begin, const std::vector<int>& gt, int k,
                          const faiss::IndexHNSWSQ* hnsw, bool use_map) {
    if (k <= 0)
        return 0.0;
    int hit = 0;
    for (int i = 0; i < k; ++i) {
...
build_or_load_hnswSQ(const std::vector<float>& base, int dim, int nb, int M, int efConstruction,
                     faiss::ScalarQuantizer::QuantizerType qtype, const std::string& dataset_name,
                     const std::string& base_path_for_tag, bool force_rebuild, long& load_ms_out, long& build_ms_out,
                     long& save_ms_out, long& train_ms_out) {
    const std::string metric = "l2";
    std::string idx_cache =
        generate_index_cache_filename_hnswsq(dataset_name, dim, nb, M, efConstruction, metric, base_path_for_tag);
...
measure_once(faiss::Index* index, int efSearch, const std::vector<float>& query_vectors, int dim, int nq, int k,
             const std::vector<std::vector<int>>& ground_truths, bool batch_mode, bool use_reorder_map) {
    RunOutcome out;
    out.ok = false;
...
append_result_csv(const std::string& dataset_name, int nb, int nq, int dim, int M, int efC, int efS, int k,
                  int warmup_rounds, int warmup_q, const std::string& reorder_algo, const RunOutcome& out) {
    ...
    f << dataset_name << "," << nb << "," << nq << "," << dim << "," << M << "," << efC << "," << efS << "," << k << ","
      << warmup_rounds << "," << warmup_q << "," << (reorder_algo.empty() ? "none" : reorder_algo) << "," << std::fixed
      << std::setprecision(2) << out.qps << "," << std::fixed << std::setprecision(2) << out.avg_latency_us << ","
      << std::fixed << std::setprecision(2) << out.p95_latency_us << "," << std::fixed << std::setprecision(2)
      << out.p99_latency_us << "," << std::fixed << std::setprecision(3) << out.avg_recall << "," << out.build_ms << ","
      << out.reorder_ms << "," << std::fixed << std::setprecision(4) << ratio << "," << out.load_ms << ","
      << out.save_ms << "," << out.gt_ms << "," << out.train_ms << "," << out.search_ms << "\n";
}
...
run_one_dataset(const std::string& name, const std::string& base_file, const std::string& query_file,
    ...
    if (!cli.reorder_algo.empty()) {
        ...
        if (perm.size() != (size_t)nb) {
            std::cerr << "  警告: reorder_perm 大小=" << perm.size() << " 与 nb=" << nb
                      << " 不一致，可能无法做ID映射\n";
        }
    }
    ...
    for (int efS : cli.efS_list) {
        ...
        //             << ", Recall_single@" << cli.k << "=" << std::fixed << std::setprecision(3) <<
        //             out_single.avg_recall
        ...
        if (do_single) {
            append_result_csv(name, nb, nq, dim, cli.M, cli.efC, efS, cli.k, cli.warmup_rounds,
                              std::min(cli.warmup_q, nq), cli.reorder_algo, out_single);
        } else if (do_batch) {
            append_result_csv(name, nb, nq, dim, cli.M, cli.efC, efS, cli.k, cli.warmup_rounds,
                              std::min(cli.warmup_q, nq), cli.reorder_algo, out_batch);
        }
...
main(int argc, char** argv) {
    try {
        ...
        if (cli.dataset.empty()) {
            ...
            if (it == presets.end()) {
                std::ostringstream oss;
                oss << "未知 --dataset: " << cli.dataset << "（可选：SIFT1M|GIST1M|ALL|CUSTOM）";
                throw std::runtime_error(oss.str());
            }
            const auto& ds = *it;
            run_one_dataset(ds.name, ds.base_file, ds.query_file, ds.gt_file, ds.dim, cli);
