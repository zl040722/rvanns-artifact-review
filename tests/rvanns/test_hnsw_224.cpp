...
static std::string
generate_index_cache_filename_hnsw(const std::string& name, int dim, int nb, int M, int efC, const std::string& metric,
    ...
    oss << index_cache_dir() << "/" << name << "_hnsw_nb" << nb << "_dim" << dim << "_M" << M << "_efC" << efC << "_"
...
// -------------------- 单位化（与 SQ 版一致） --------------------
static void
l2_normalize_inplace(std::vector<float>& data, int n, int d) {
    for (int i = 0; i < n; ++i) {
...
get_groundtruth(const std::vector<float>& base_vectors, const std::vector<float>& query_vectors, int dim, int nb,
                int nq, int k, const std::string& dataset_name,
...
// -------------------- metrics --------------------
static double
recall_at_k_one(const std::vector<faiss::idx_t>& pred, const std::vector<int>& gt, int k) {
    if ((int)pred.size() < k || (int)gt.size() < k)
        return 0.0;
    int hit = 0;
    for (int i = 0; i < k; i++) {
...
static double
percentile_sorted(const std::vector<double>& sorted_latencies, double percentile) {
...
// -------------------- 索引构建/加载（HNSW-IP） --------------------
static std::unique_ptr<faiss::Index>
build_or_load_hnswIP(const std::vector<float>& base, int dim, int nb, int M, int efConstruction,
                     const std::string& dataset_name, const std::string& base_path_for_tag, long& load_ms_out,
                     long& build_ms_out, long& save_ms_out, long& train_ms_out) {
    const std::string metric = "l2";  // 与 SQ 版保持一致，避免混入“未单位化”的旧缓存
    std::string idx_cache =
        generate_index_cache_filename_hnsw(dataset_name, dim, nb, M, efConstruction, metric, base_path_for_tag);
    ...
    std::cout << "开始构建 HNSW(IP) 索引 (nb=" << nb << ", dim=" << dim << ", M=" << M << ", efC=" << efConstruction
              << ")...\n";
...
static RunOutcome
measure_once(faiss::Index* index, int efSearch, const std::vector<float>& query_vectors, int dim, int nq, int k,
...
append_result_csv(const std::string& dataset_name, int nb, int nq, int dim, int M, int efC, int efS, int k,
    ...
    if (!f)
        return;
    if (need_header) {
        f << "dataset,nb,nq,dim,M,efC,efS,k,warmup_rounds,warmup_q,qps,avg_us,p95_us,p99_us,recall,build_ms,load_ms,"
             "save_ms,gt_ms,train_ms,search_ms\n";
    }
    f << dataset_name << "," << nb << "," << nq << "," << dim << "," << M << "," << efC << "," << efS << "," << k << ","
      << warmup_rounds << "," << warmup_q << "," << std::fixed << std::setprecision(2) << out.qps << "," << std::fixed
      << std::setprecision(2) << out.avg_latency_us << "," << std::fixed << std::setprecision(2) << out.p95_latency_us
      << "," << std::fixed << std::setprecision(2) << out.p99_latency_us << "," << std::fixed << std::setprecision(3)
      << out.avg_recall << "," << out.build_ms << "," << out.load_ms << "," << out.save_ms << "," << out.gt_ms << ","
      << out.train_ms << "," << out.search_ms << "\n";
}
...
all_presets() {
    return {{"SIFT1M", "vectors_out/sift1m_base.bin", "vectors_out/sift1m_query.bin",
             "vectors_out/sift1m_groundtruth.ivecs", 128},
            {"GIST1M", "vectors_out/gist1m_base.bin", "vectors_out/gist1m_query.bin",
             "vectors_out/gist1m_groundtruth.ivecs", 960},
            {"GloVe1M", "vectors_out/glove1m_base.bin", "vectors_out/glove1m_query.bin",
             "vectors_out/glove1m_groundtruth.ivecs", 100}};
}

// -------------------- 跑一个数据集 --------------------
static void
run_one_dataset(const std::string& name, const std::string& base_file, const std::string& query_file,
                const std::string& gt_file, int dim, const Cli& cli) {
    std::cout << "\n==== 数据集: " << name << " ====\n";
    ...
    auto index =
        build_or_load_hnswIP(base, dim, nb, cli.M, cli.efC, name, base_file, load_ms, build_ms, save_ms, train_ms);
    ...
    for (int efS : cli.efS_list) {
        ...
        std::cout << "[efS=" << efS << "] ";
        if (do_single) {
            std::cout << "QPS_single=" << std::fixed << std::setprecision(2) << out_single.qps << ", Recall_single@"
                      << cli.k << "=" << std::fixed << std::setprecision(3) << out_single.avg_recall
        ...
        if (do_batch) {
            if (do_single)
                std::cout << " | ";
            std::cout << "QPS_batch=" << std::fixed << std::setprecision(2) << out_batch.qps << ", Recall_batch@"
                      << cli.k << "=" << std::fixed << std::setprecision(3) << out_batch.avg_recall
                      << " (batch，延迟N/A)";
        }
        std::cout << "\n";

        if (do_single) {
            append_result_csv(name, nb, nq, dim, cli.M, cli.efC, efS, cli.k, cli.warmup_rounds,
                              std::min(cli.warmup_q, nq), out_single);
        } else if (do_batch) {
            append_result_csv(name, nb, nq, dim, cli.M, cli.efC, efS, cli.k, cli.warmup_rounds,
                              std::min(cli.warmup_q, nq), out_batch);
        }
...
main(int argc, char** argv) {
    try {
        ...
        if (cli.dataset.empty()) {
            ...
            if (it == presets.end()) {
                std::ostringstream oss;
                oss << "未知 --dataset: " << cli.dataset << "（可选：SIFT1M|GIST1M|GloVe1M|ALL|CUSTOM）";
                throw std::runtime_error(oss.str());
            }
            const auto& ds = *it;
            run_one_dataset(ds.name, ds.base_file, ds.query_file, ds.gt_file, ds.dim, cli);
